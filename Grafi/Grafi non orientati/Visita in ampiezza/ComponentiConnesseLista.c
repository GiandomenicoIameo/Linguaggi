...
void inserimentoAdiacenze( Coda** Testa, Grafo* graph, int verticeSrc, int* arrayVisite, int* predecessori );
int verificaVertici( Grafo* graph, int verticeSrc );
void visitaInAmpiezza( Grafo* graph, int numVertice, int verticeSrc, Coda** Testa, int* predecessori );
void stampaPercorso( int verticeSrc, int verticeDst, int* predecessori  );
void generazioneComponentiConnesse( int verticeSrc, int numVertice, int* predecessori );
...
int main( void ) {
  ...
  stampaLista( graph );
  puts( "" );
  visitaInAmpiezza( graph, vertice, 0, &Testa, predecessori );
  generazioneComponentiConnesse( 0, vertice, predecessori );
  ...
  if( arrayVisite[ verticeSrc ] == 0 ) {
    ...
    while( graph->summit[ verticeSrc ] != NULL ) {
        if( !arrayVisite[ graph->summit[ verticeSrc ]->key ] ) {
          ...
          predecessori[ graph->summit[ verticeSrc ]->key ] = verticeSrc;
        }
        graph->summit[ verticeSrc ] = graph->summit[ verticeSrc ]->next;
    }
}
...
void visitaInAmpiezza( Grafo* graph, int numVertice, int verticeSrc, Coda** Testa, int* predecessori ) {
  ...
  if( verificaVertici( graph, verticeSrc ) )
      inserimentoAdiacenze( Testa, graph, verticeSrc, arrayVisite, predecessori );
  else {
    for( int indice = 0; indice < graph->numeroVertici; indice++ ) {
         if( verificaVertici( graph, indice ) ) {
             inserimentoAdiacenze( Testa, graph, indice, arrayVisite, predecessori );
             break;
      }
    }
  }
  ...
  while( *Testa != NULL ) {
    if( arrayVisite[ ( *Testa )->dato ] == 2 ) {
      ...
      inserimentoAdiacenze( Testa, graph, ( *Testa )->dato, arrayVisite, predecessori );
    }
  }
  free( arrayVisite );
}
...
void generazioneComponentiConnesse( int verticeSrc, int numVertice, int* predecessori ) {
     ...
     for( int indice = 0; indice < numVertice; indice++ ) {
          stampaPercorso( verticeSrc, indice, predecessori );
          puts( "" );
     }
}
